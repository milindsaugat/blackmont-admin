...
export default function BlogEditor({ data, onChange, globalFeaturedCount }) {
  ...
  const removeArticle = (index) => {
    if (window.confirm("Deleting this article will remove it permanently. Continue?")) {
  ...
  const updateArticle = (index, field, val) => {
    ...
    // Check global limit when turning on featured
    if (field === 'isFeatured' && val === true && !newItems[index].isFeatured) {
      if (globalFeaturedCount >= 3) {
        alert("Only 3 articles can be featured on the home page across all insights. Disable another first.");
        return;
      }
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      {/* PART 3: BLOG ARTICLES CRUD */}
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h4 className="typo-label" style={{ margin: 0, color: 'var(--gold)' }}>PART 2: BLOG ARTICLES</h4>
          <div style={{ background: 'rgba(212,175,55,0.1)', padding: '6px 12px', borderRadius: 20, border: '1px solid var(--gold-30)' }}>
            <span style={{ fontSize: 12, color: 'var(--gold)', fontWeight: 600 }}>View Featured ({globalFeaturedCount}/3)</span>
          </div>
        </div>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {article.isFeatured && <span style={{ color: 'var(--gold)', fontSize: 14 }}>⭐</span>}
                        <span style={{ flex: 1, fontSize: 14, fontWeight: 500, color: article.title ? '#fff' : '#666' }}>
                          {article.title || 'Untitled Article'}
                        </span>
                      </div>
                      <div style={{ display: 'flex', gap: 12, fontSize: 12, color: 'var(--text-muted)' }}>
                        <span style={{ color: 'var(--gold)' }}>{article.categoryTag || 'NO CATEGORY'}</span>
                        <span>•</span>
                    ...
                    <div style={{ padding: '0 20px 20px 20px', borderTop: '1px solid var(--border-default)' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 20, marginTop: 20 }}>
                          ...
                          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 16 }}>
                            ...
                            <div style={{ display: 'flex', gap: 16 }}>
                              <div style={{ flex: 1 }}>
                                ...
                                <input type="text" value={article.categoryTag || ''} onChange={e => updateArticle(index, 'categoryTag', e.target.value.toUpperCase())} style={iStyle} onFocus={focus} onBlur={blur} onMouseEnter={hover} onMouseLeave={unhover} placeholder="e.g. MARKET INSIGHT" />
                              </div>
                              <div style={{ flex: 1 }}>
                              ...
                              <div style={{ flex: 1 }}>
                                <label className="typo-label" style={{ marginBottom: 8, display: 'block' }}>Read Time</label>
                                <input type="text" value={article.readTime || ''} onChange={e => updateArticle(index, 'readTime', e.target.value)} style={iStyle} onFocus={focus} onBlur={blur} onMouseEnter={hover} onMouseLeave={unhover} placeholder="e.g. 5 min read" />
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <input type="text" value={article.slug || ''} onChange={e => updateArticle(index, 'slug', e.target.value)} style={iStyle} onFocus={focus} onBlur={blur} onMouseEnter={hover} onMouseLeave={unhover} placeholder="/insights/article-name or https://..." />
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-root)', padding: '16px 20px', borderRadius: 8, border: '1px solid var(--border-default)' }}>
                          ...
                          <div style={{ display: 'flex', gap: 32 }}>
                            {/* Feature Toggle */}
                            <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={(e) => { e.preventDefault(); updateArticle(index, 'isFeatured', !article.isFeatured); }}>
                              <div style={{ width: 40, height: 20, borderRadius: 12, background: article.isFeatured ? 'var(--gold)' : 'var(--bg-inset)', border: `1px solid ${article.isFeatured ? 'var(--gold-20)' : 'var(--border-default)'}`, position: 'relative', transition: 'all 200ms' }}>
                                <div style={{ width: 14, height: 14, borderRadius: '50%', background: article.isFeatured ? '#000' : 'var(--text-muted)', position: 'absolute', top: 2, left: article.isFeatured ? 22 : 2, transition: 'all 200ms' }} />
                              </div>
                              <span style={{ color: 'var(--text-primary)', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>⭐ Feature on Home Page</span>
                            </label>
